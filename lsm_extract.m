function LSM =lsm_extract(data,M,N)
% Extract random LSM's at R=3 at different local regions
% Input: 
%       data: spectral pixel set
%       M: number of blocks in row level
%       N: number of blocks in column level
%              
% Output:
%       LSM: Pixel Difference Vectors at R=3


% extract LSM samples from data
...
for i=1:M
    for j=1:N
        LSM{i,j} = zeros(randnum1*randnum2*size(data,3),size(nn,1));
    end
end
cnt = 0;
...
for d=1:size(data,3)
    ...
    for i = 1:M
        ...
        for j = 1:N
            ...
            LSM{i,j}(cnt+1:cnt+randnum1*randnum2,:) = ...
